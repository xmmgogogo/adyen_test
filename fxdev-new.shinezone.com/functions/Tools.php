...
// 得到区间内的所有天数信息
function getLineDay2($start, $end)
{
    $days = [];
    while (true) {
        $days[] = $start;
...
// 得到区间内的所有天数信息
function getLineMonth($start, $end)
{
    $months = [];
    while (true) {
        if (strtotime($start) >= strtotime($end)) {
        ...
        $months[] = date('Y-m-01', strtotime($start));
...
// 得到区间内的所有天数信息
function getLineWeek($start, $end)
{
    $weeks = [];
    while (true) {
        if (strtotime($start) >= strtotime($end)) {
        ...
        $weeks[] = date('Y-m-d', strtotime($start));
        $start    = date('Y-m-d', strtotime("$start +1 week"));
    }
    return $weeks;
}

function getLineHour($start, $end)
{
    $hours = [];
    while (true) {
        if (strtotime($start) >= strtotime($end)) {
        ...
        $hours[]  = date('Y-m-d H:00:00', strtotime($start));
        $start    = date('Y-m-d H:00:00', strtotime("$start +1 hour"));
    }
    return $hours;
}
function getLineMinute($start, $end)
{
    $minutes = [];
    while (true) {
        if (strtotime($start) >= strtotime($end)) {
        ...
        $minutes[]= date('Y-m-d H:i:00', strtotime($start));
        $start    = date('Y-m-d H:i:00', strtotime("$start +1 minute"));
    }
    return $minutes;
}
...
//granularity:hour
//startdate:2017-01-01 07:00:00
//enddate:2017-01-02 07:00:00
//granularity:week
//startdate:2017-01-01 00:00:00
//enddate:2017-01-29 00:00:00
//granularity:minute
//startdate:2017-01-16 07:00:00
//enddate:2017-01-16 08:00:00
function getTransactionDate()
{
    //echo date('Y-m-d H:i:s', 1484521200);die;
    $post        = getPostInfo();

    $granularity = isset($post['granularity'])?$post['granularity']:'minute';//'hour';//'week';//'month';//'day';//month hour week minute
    $dateArr = [];
    if ($granularity == 'day') {
        $start       = isset($post['startdate'])?$post['startdate']:'2017-01-17 00:00:00';
        $end         = isset($post['enddate'])?$post['enddate']:'2017-01-24 00:00:00';
        $endTime     = date('Y-m-d 23:59:59', strtotime("$end -1 day"));
        $dateArr['selectedPeriod']   = [
        ...
        //echo $times;die;
        $dateArr['previousPeriod']   = [
            'start'  => date('Y-m-d 00:00:00', strtotime("$start -$times day")),
            'end'    => date('Y-m-d 23:59:59', strtotime("$start -1 day")),
        ];
        //var_dump($dateArr);die;
    } else if($granularity == 'hour') {
        $start       = isset($post['startdate'])?$post['startdate']:'2017-01-01 07:00:00';
        $end         = isset($post['enddate'])?$post['enddate']:'2017-01-02 07:00:00';
        $endTime     = date('Y-m-d H:59:59', strtotime("$end -1 hour"));
        $dateArr['selectedPeriod']   = [
            'start'  => date('Y-m-d H:00:00', strtotime($start)),
            'end'    => date('Y-m-d H:59:59', strtotime($endTime))
        ];
        $times       = 0;
        while(strtotime($end) > strtotime($start)) {
            $end     = date('Y-m-d H:00:00', strtotime("$end -1 hour"));
            $times++;
        }
        //echo $times;die;
        $dateArr['previousPeriod']   = [
            'start'  => date('Y-m-d H:00:00', strtotime("$start -$times hour")),
            'end'    => date('Y-m-d H:59:59', strtotime("$start -1 hour")),
        ];
        //var_dump($dateArr);die;
    } else if($granularity == 'minute') {
        $start       = isset($post['startdate'])?$post['startdate']:'2017-01-01 07:00:00';
        $end         = isset($post['enddate'])?$post['enddate']:'2017-01-01 08:00:00';
        $endTime     = date('Y-m-d H:i:59', strtotime("$end -1 minute"));
        $dateArr['selectedPeriod']   = [
            'start'  => date('Y-m-d H:i:00', strtotime($start)),
            'end'    => date('Y-m-d H:i:59', strtotime($endTime))
        ];
        $times       = 0;
        while(strtotime($end) > strtotime($start)) {
            $end     = date('Y-m-d H:i:00', strtotime("$end -1 minute"));
            $times++;
        }
        //echo $times;die;
        $dateArr['previousPeriod']   = [
            'start'  => date('Y-m-d H:i:00', strtotime("$start -$times minute")),
            'end'    => date('Y-m-d H:i:59', strtotime("$start -1 minute")),
        ];
        //var_dump($dateArr);die;
    } else if($granularity == 'week') {
        $start       = isset($post['startdate'])?$post['startdate']:'2017-01-01 00:00:00';
        $end         = isset($post['enddate'])?$post['enddate']:'2017-01-22 00:00:00';
        $endTime     = date('Y-m-d', strtotime("$end -1 day"));
        $dateArr['selectedPeriod']   = [
            'start'  => date('Y-m-d 00:00:00', strtotime($start)),
            'end'    => date('Y-m-d 23:59:59', strtotime($endTime))
        ];
        $times       = 0;
        while(strtotime($end) > strtotime($start)) {
            $end     = date('Y-m-d 00:00:00', strtotime("$end -1 week"));
            $times++;
        }
        //echo $times;die;
        $dateArr['previousPeriod']   = [
            'start'  => date('Y-m-d 00:00:00', strtotime("$start -$times week")),
            'end'    => date('Y-m-d 23:59:59', strtotime("$start -1 day")),
        ];
    } else if($granularity == 'month') {
        $start       = isset($post['startdate'])?$post['startdate']:'2016-12-01 00:00:00';
        $end         = isset($post['enddate'])?$post['enddate']:'2017-02-01 00:00:00';
        $endTime     = date('Y-m-d', strtotime("$end -1 day"));
        $dateArr['selectedPeriod']   = [
            'start'  => date('Y-m-d 00:00:00', strtotime($start)),
            'end'    => date('Y-m-d 23:59:59', strtotime($endTime))
        ...
        $times       = 0;
        while(strtotime($end) > strtotime($start)) {
            $end     = date('Y-m-d 00:00:00', strtotime("$end -1 month"));
            $times++;
        }
        //echo $times;die;
        $dateArr['previousPeriod']   = [
            'start'  => date('Y-m-d 00:00:00', strtotime("$start -$times month")),
            'end'    => date('Y-m-d 23:59:59', strtotime("$start -1 day")),
        ];
    } else {
        $times       = 0;
...
function formatReportDisplayList($list)
{
    $downloadList = '';
    if ($list) {
        ...
        {
            $size = 0;
            if (is_file(DOWNLOAD_PATH . $item['FileName']))
            {
                $filename = DOWNLOAD_PATH . $item['FileName'];
                $size     = filesize($filename);
            }
            $downloadList .= '<tr><td>';
            $downloadList .= '<a href="/ca/ca/reports/download.php?fileName='.$item['FileName'].'"
                                 title="'.$item['FileName'].'">'.$item['FileName'].'</a></td>';
            $downloadList .= '<td>'.$item['CreateDate'].' CET</td>';
            $downloadList .= '<td class="align-right" title="'.$size.' bytes">'.sizeFormat($size).'</td></tr>';
        }
    }
    return $downloadList;
}
function downloadFile($fileName)
{
    ...
    switch ($extension) {
        ...
        case 'zip':
            ...
            break;
        default :
            $contentType = 'force-download';
    }
    $fileRealName = DOWNLOAD_PATH . $fileName;
    if (file_exists($fileRealName))
    {
        $size     = filesize($fileRealName);
        header("Content-type: $contentType; charset=UTF-8");
        header("Accept-Ranges: bytes");
        header("Accept-Length: ".strlen($size));
        header("Content-Disposition: attachment; filename=" . $fileName);

        @readfile($fileRealName);
        //echo file_get_contents($fileRealName);
    }
}
function generateFileExtension($format)
{
    switch ($format) {
        case 'CSV' :
        case 'XML' :
        case 'PDF' :
            $fileExtention = strtolower($format);
            break;
        case 'XLS_GEN' :
            $fileExtention = 'xlsx';
            break;
