define("charts/acquirerConversion/js/collections/acquirerAccountData",["jquery","underscore","backbone","charts/acquirerConversion/js/models/conversion","chartlib/events/appstateevents","chartlib/chartBaseEvents","chartutil/numberutils"],function(f,b,h,a,d,g,e){var c=h.Collection.extend({model:a,reverseDirection:true,conversionTypes:["Authorised","Refused"],hasParsedData:false,hasData:false,statsType:"CONVERSION_PER_ACQUIRER_ACCOUNT",initialize:function(j,i){b.bindAll(this,"parse","loadData","onLoaded","onError","getReverseDirection");
...
i.statsType=this.statsType;var j=k.get("label").toLowerCase();if(j.length){j=j+"/";}if(k.get("stub")){return adyen.jsbase+"/chart/charts/acquirerConversion/dummy-data/"+j+"conversion_per_acquirer_account.xml?"+new Date().getTime();
}else{return k.get("url")+f.param(i);}},fetch:function(i){i||(i={});i.dataType="xml";i.success=this.onLoaded;i.error=this.onError;
this._super(i);h.trigger(g.REGISTER_DEFERRED,this.loaded,"AcquirerAccountConversionChartData",this);},parse:function(m){this.xmlData=m;
var j=f(this.xmlData).find("set");this.volDict={};var l=this,k;var i=0;j.each(function(p,t){var s=f(t);var o=s.attr("toolText").match(/,\s([^)]+)\s/)[1];
var n=s.attr("toolText").match(/([^,]+)/)[0].toLowerCase();var r=s.attr("toolText").match(/(,)[^,]*$/)[0];var q=r.match(/\(([^)]+)\)/)[0];
q=Number(q.match(/\d+/g)[0]);i=i+q;l.volDict[o]||(l.volDict[o]=new a());l.volDict[o].set("name",o);l.volDict[o].set("total",l.volDict[o].get("total")+Math.max(0,+q));
...
this.hasParsedData=true;if(this.volDict.length){this.sortBy("percentage");this.hasData=true;}else{this.trigger(g.DATA_FAILED);
